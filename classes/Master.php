...
Class Master extends DBConnection {
	...
	function save_student(){
		...
		$check = $this->conn->query("SELECT * FROM `student_list` where `fullname` = '{$fullname}' and id = '{$id}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
	...
	function save_class(){
		...
		$check = $this->conn->query("SELECT * FROM `class_list` where `name` = '{$name}' and user_id = '{$uid}' and `student_id` = '{$student_id}' and user_id = '{$this->settings->userdata('id')}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Class already exists on the selected student.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
